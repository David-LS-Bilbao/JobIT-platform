...
- **`LEGAL_INTERPRETATION_REQUIRED`**: la implementación solicitada depende de transformar un estado `PRELIMINARY`, `CONDITIONAL`, `TO_VERIFY`, `RECOMMENDATION` o `HUMAN_REVIEW_REQUIRED` de la baseline jurídica preparatoria en `LEGAL_REQUIREMENT`, `APPROVED` o `COMPLIANT`. Esa transformación corresponde a la revisión jurídica humana, nunca al ejecutor. Como cualquier otra condición de esta lista, la acción es `FULL_STOP`: se detiene toda la ejecución, no se modifica ningún archivo más (`FURTHER_FILESYSTEM_MUTATION: NO`), se emite `BLOCKED` y solo se reanuda tras una decisión humana explícita. No existe parada parcial ni continuación del alcance restante.
- **`LEGAL_REFERENCE_MUTATION_NOT_AUTHORIZED`**: se intenta crear, editar o borrar cualquier archivo bajo `docs/legal/reference/**` sin autorización documental explícita en el brief. La baseline es `PREPARATORY_REFERENCE_ONLY` y se consulta, no se corrige sobre la marcha.
- **`PRIVATE_LEGAL_DATA_IN_VERSION_CONTROL`**: aparecen en el repositorio, sin autorización explícita, identidad privada completa del responsable, identificador fiscal, domicilio privado, credenciales, secrets o claves de API, contratos reales o completos con encargados, valores privados de proveedores, solicitudes reales de derechos, incidentes o brechas reales, evidencias reales de consentimiento o aceptación, datos reales de personas candidatas, nombres privados de personal autorizado, o configuraciones de seguridad explotables. El repositorio contiene el modelo; el entorno privado contiene los valores.
...
- [docs/agents/skills/privacy-legal-reference.md](skills/privacy-legal-reference.md): define el `PRIVACY_IMPACT_PRECHECK` y las reglas de consulta de la baseline jurídica preparatoria; es el documento que detecta las tres condiciones especializadas de privacidad listadas arriba.
